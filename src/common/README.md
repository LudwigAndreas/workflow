...
**Placeholder.** Shared contracts and types (API/event schemas, generated clients) consumed by frontend and backend as a versioned package. Not a submodule dependency of either — see the workflow repo's AGENTS.md for why.

This is a reference scaffold, not a real repository yet — it has no git
remote of its own. When the real `common` repository exists, promote this
placeholder from the workflow repo root:

```bash
scripts/add-repo.sh common <git-url>
```
