...
**Placeholder.** Deployment manifests and pipelines for the backend service.

This is a reference scaffold, not a real repository yet — it has no git
remote of its own. When the real `gitops_backend` repository exists, promote this
placeholder from the workflow repo root:

```bash
scripts/add-repo.sh gitops_backend <git-url>
```
